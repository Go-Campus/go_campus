...
// Format form data for submission - matches reference implementation exactly
export const formatFormDataForSubmission = (formData) => {
  ...
  Object.keys(formData).forEach(key => {
    ...
    } else if (key === 'authenticationId' && typeof value === 'object' && value !== null) {
      // Convert authenticationId to match reference format: {country: number, number: string, numberLength: number}
      const mobileData = value;
      if (mobileData.country && mobileData.number) {
        const countryCode = String(mobileData.country).replace('+', ''); // Remove + prefix
        const phoneNumber = String(mobileData.number);
        const numberLength = phoneNumber.replace(/\D/g, '').length; // Count only digits
        ...
        formDataPayload.append(key, JSON.stringify(authIdObject));
      }
